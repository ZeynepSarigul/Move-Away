...
class NewCostOfLivingComponent extends React.Component {
  ...
  //change the currency type once the ~~ button is clicked switch between new and current city
  changeCurrencyTypeAndValue = () => {
    let originalCurrencyType = this.props.currencyType;
    let originalValue = this.props.exactNewCostOfLiving;
    let newCurrencyType;

    const dataSet = require("../data/cost_of_living_indices.json");
    // here we switch the type of currency
    if (
      this.props.currencyType === dataSet[this.props.currentCity].currency_type
    ) {
      newCurrencyType = dataSet[this.props.newCity].currency_type;
    } else if (
      this.props.currencyType === dataSet[this.props.newCity].currency_type
    ) {
      newCurrencyType = dataSet[this.props.currentCity].currency_type;
    }

    // here we calculate the new values based on exchange rates.
    let exactValue =
      (originalValue / this.state.currencyResponseRates[originalCurrencyType]) *
      this.state.currencyResponseRates[newCurrencyType];

    this.props.changeCurrencyTypeAndValue(
      exactValue,
      (Math.round(exactValue / 100) * 100)
        .toString()
        .replace(/\B(?=(\d{3})+(?!\d))/g, ","),
      newCurrencyType
    );
  ...
  isASpotAHomeCity = () => {
    if (
      this.props.newCity === "Madrid, Spain" ||
  ...
  componentDidMount() {
    fetch(
      "https://openexchangerates.org/api/latest.json?app_id=0cc840f2153c4d378b1a2687918435e7"
    )
      .then(response => {
        if (!response.ok) {
          throw Error(
            "Something went wrong retreiving currency information :("
          );
        }
        ...
        if (this.props.newCitySlug) {
          fetch(
            ...
            .then(response => {
              if (!response.ok) {
                throw Error(
                  "Something went wrong retreiving city information :("
                );
              }
              return response.json();
            })
            .then(responseData => {
              let randomObject =
                responseData.salaries[
                  Math.floor(Math.random() * responseData.salaries.length)
                ];
              let randomPosition = randomObject.job.title;
              let randomSalary = randomObject.salary_percentiles.percentile_50;

              let configuredSalary =
                (randomSalary / this.state.currencyResponseRates["USD"]) *
                this.state.currencyResponseRates[this.state.targetCurrency];
              let roundedConfiguredSalary = (
                Math.round(configuredSalary / 100) * 100
              )
                .toString()
                .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    ...
    if (this.props.newCitySlug) {
      ...
      fetch(
        ...
        .then(response => {
          if (!response.ok) {
            throw Error("Something went wrong retreiving city scores:(");
          }
          return response.json();
        })
        .then(responseData => {
          this.setState({
            healthCareScore: responseData.categories[9].score_out_of_10
          });
        })
        .catch(error => {
          console.log(error);
  ...
  render() {
    ...
    const dataSet = require("../data/cost_of_living_indices.json");
    // find if percent changes are negative or not
    ...
    if (this.props.purchasingPowerPercentChange > 0) {
      purchasingPowerPercentChange = this.props.purchasingPowerPercentChange;
      purchasing_increase_format = true;
    } else if (this.props.purchasingPowerPercentChange <= 0) {
      purchasingPowerPercentChange = -this.props.purchasingPowerPercentChange;
    ...
    return (
      <div style={full_page_height}>
        <script src="https://kit.fontawesome.com/3eacea46fb.js"></script>
          ...
          <div>
            <div style={buttons_container}>
              <button
                ...
                onClick={this.props.resetToFirstStep}
              >
                Menu
              </button>
            </div>
            {this.props.currencyType ===
              dataSet[this.props.currentCity].currency_type && (
              <div>
                <div
                  className="search_question_container tooltip-left"
                  data-tooltip={
                    "Pick another city to compare its Cost of Living to living in -  " +
                    this.props.currentCity +
                    " with a net income of " +
                    this.state.currentCurrency +
                    " " +
                    this.props.currentCostOfLiving +
                    "."
                  }
                >
                  <i
                    style={information_icon}
            ...
            {this.props.currencyType !==
              dataSet[this.props.currentCity].currency_type && (
              <div
                style={information_icon_container}
                className="tooltip-left"
                data-tooltip="Convert back to your base currency to be able compare the Cost of Living with another city!"
              >
                <i
                  style={information_icon}
            ...
            <div style={full_page_height}>
              <div style={image_container} className="background-darken">
                {this.props.newCitySlug && (
                  <img
                    ...
                    style={{ width: "100%", opacity: "0.9", height: "40vh" }}
                    alt="City Banner"
                  />
                )}
                {!this.props.newCitySlug && (
                  <img
                    ...
                    style={{ width: "100%", opacity: "0.7", height: "30vh" }}
                    alt="Stock Banner"
                  />
                )}
                <div>
                  {this.props.newCitySlug && (
                    <div className="banner-image-intro">
                      <br />
                      {this.props.newCity}
                      <br />
                      {this.props.newCitySlug !== "london" &&
                        this.props.newCitySlug !== "moscow" &&
                        this.props.newCitySlug !== "brussels" && (
                          <span className="banner-image-sub-intro">
                            {this.state.bannerIntro}.
                          </span>
                        )}
                      {this.props.newCitySlug === "london" && (
                        <span className="banner-image-sub-intro">
                          London is one of the world's most inviting cities for
                          startups, as well as home to world-class schools,
                          universities and museums.
                        </span>
                      )}
                      {this.props.newCitySlug === "moscow" && (
                        <span className="banner-image-sub-intro">
                          Moscow is a lively city, rich in history and culture
                          and is Russia's national center for visual and
                          performing arts.
                        </span>
                      )}
                      {this.props.newCitySlug === "brussels" && (
                        <span className="banner-image-sub-intro">
                          Life in Brussels is a culturally rich,
                          central-European adventure, home to much of the
                  ...
                  {!this.props.newCitySlug && (
                    <div className="alternative-banner-image-intro">
                      {this.props.newCity}
                    </div>
                  )}
                </div>
              ...
              <div style={middle_container} className="container">
                <p style={intro}>
                  To have the same standard of living, a comparable salary would
                  be
                </p>
                {this.state.currentCurrency !== this.state.targetCurrency && (
                  <div>
                    {this.props.newCitySlug && (
                      <p style={comparable_salary_text}>
                        ≈ {this.props.currencyType} {this.props.value}
                        <sub
                          style={salary_sub_properties}
                    ...
                    {!this.props.newCitySlug && (
                      <p style={alternative_comparable_salary_text}>
                        ≈ {this.props.currencyType} {this.props.value}
                        <sub
                          style={salary_sub_properties}
                          className="tooltip-bottom"
                ...
                {this.state.currentCurrency === this.state.targetCurrency && (
                  <div>
                    ...
                    {!this.props.newCitySlug && (
                      <p
                        style={alternative_comparable_salary_text}
                        onClick={this.changeCurrencyTypeAndValue}
                      >
                        ≈ {this.props.currencyType} {this.props.value}
                      </p>
                ...
                <div className="row">
                  {!this.isASpotAHomeCity() && (
                    <div className="col-xs-12 col-sm-6 col-md-6 col-lg-3 mobilePadding">
                      ...
                      {!rent_increase_format &&
                        this.props.rentPercentChange === 0 && (
                          <p
                            style={percent_index_text_no_change}
                            className="tooltip-bottom"
                            data-tooltip={"Rent is about the same!"}
                          >
                            {" "}
                            It's the same!
                          </p>
                  ...
                  {this.isASpotAHomeCity() && (
                    <div className="col-xs-12 col-sm-6 col-md-6 col-lg-3 mobilePadding">
                      <div
                        onClick={this.openSpotAHomeDetails}
                        style={icon_index}
                        id="spotahome-icon-box"
                      >
                        <span>
                          <i className="fa fa-strikethrough fa fa-spotahome-lg-modification"></i>
                        </span>
                      </div>
                      <p
                        onClick={this.openSpotAHomeDetails}
                        style={sub_salary_text}
                        className="tooltip-top"
                        data-tooltip={
                          "Let Spotahome help in your move to " +
                          this.props.newCity +
                          "!"
                        }
                      >
                  ...
                  <div className="col-xs-12 col-sm-6 col-md-6 col-lg-3 mobilePadding">
                    <div style={icon_index} id="icon-box">
                      <span>
                        <i className="fa fa-shopping-cart fa fa-lg-modification"></i>
                      </span>
                    </div>
                    <p style={sub_salary_text}>Groceries</p>
                    {groceries_increase_format && (
                      <p
                        style={percent_index_text}
                        className="tooltip-bottom"
                        data-tooltip={
                    ...
                    {!groceries_increase_format &&
                      this.props.groceriesPercentChange === 0 && (
                        <p
                          style={percent_index_text_no_change}
                          className="tooltip-bottom"
                          data-tooltip={"Groceries are about the same!"}
                        >
                          {" "}
                          It's the same!
                        </p>
                  ...
                  <div className="col-xs-12 col-sm-6 col-md-6 col-lg-3 mobilePadding">
                    <div style={icon_index} id="icon-box">
                      <span>
                        <i className="fa fa-cutlery fa fa-lg-modification"></i>
                      </span>
                    </div>
                    <p style={sub_salary_text}>Dining Out</p>
                    {restaurant_increase_format && (
                      <p
                        style={percent_index_text}
                        className="tooltip-bottom"
                        data-tooltip={
                          "Dining out is " +
                          restaurantPercentChange +
                          "% more expensive in " +
                          this.props.newCity +
                          "!"
                        }
                    ...
                    {!restaurant_increase_format &&
                      this.props.restaurantPercentChange !== 0 && (
                        <p
                          style={percent_index_text}
                          className="tooltip-bottom"
                          data-tooltip={
                            "Dining out is cheaper by " +
                            restaurantPercentChange +
                            "% in " +
                            this.props.newCity +
                            "."
                    ...
                    {!restaurant_increase_format &&
                      this.props.restaurantPercentChange === 0 && (
                        <p
                          style={percent_index_text_no_change}
                          className="tooltip-bottom"
                          data-tooltip={"Dining out is about the same!"}
                        >
                          {" "}
                          It's the same!
                        </p>
                  ...
                  <div className="col-xs-12 col-sm-6 col-md-6 col-lg-3 mobilePadding">
                    <div style={icon_index} id="icon-box">
                      <span>
                        <i className="fa fa-star fa fa-lg-modification"></i>
                      </span>
                    </div>
                    <p style={sub_salary_text}>Purchasing Power</p>
                    {purchasing_increase_format && (
                      <p
                        style={percent_index_text}
                        className="tooltip-bottom"
                        data-tooltip={
                          "Purchasing Power (i.e. the number of of goods/services that can be purchased by a unit of currency) is " +
                    ...
                    {!purchasing_increase_format &&
                      this.props.purchasingPowerPercentChange !== 0 && (
                        <p
                          style={percent_index_text}
                          className="tooltip-bottom"
                          data-tooltip={
                    ...
                    {!purchasing_increase_format &&
                      this.props.purchasingPowerPercentChange === 0 && (
                        <p
                          ...
                          className="tooltip-bottom"
                          data-tooltip={
                            "The purchasing power is about the same!"
                          }
                        >
                          {" "}
                          It's the same!
        ...
        {this.state.openSpotAHomeDetails && (
          <SpotAHomeDetailsComponent
            closeSpotAHomeDetails={this.closeSpotAHomeDetails}
